...
public class VisualClaim implements Listener {
    ...
    public static final Material[] MAP_MATERIALS = { Material.DIAMOND_BLOCK,
            Material.GOLD_BLOCK, Material.OAK_LOG, Material.BRICKS, Material.OAK_PLANKS,
    ...
    public void draw(boolean silent) {
        ...
        switch (type) {
            case MAP, SURFACE_MAP -> currentMaps.put(player.getName(), this);
            case SUBCLAIM_MAP -> currentSubclaimMaps.put(player.getName(), this);
            default -> visualClaims.put(player.getName(), this);
        }
        ...
        switch (type) {
            case CREATE -> {
                ...
                if (!bypass && HCF.getInstance().getMapHandler().isKitMap()) {
                    if (!silent) {
            ...
            case SURFACE_MAP -> {
                for (Entry<Claim, Team> regionData : LandBoard.getInstance().getRegionData(player.getLocation(), MAP_RADIUS, 256, MAP_RADIUS)) {
                    ...
                    for (Coordinate coordinate : claim) {
                        ...
                        while (!block.getType().isSolid() || block.getType() == Material.OAK_LEAVES || block.getType() == Material.OAK_LOG || block.getType() == Material.OAK_LOG) {
                        ...
                        sendBlockChange(player, block.getLocation(), Material.WHITE_WOOL, carpetColor.getWoolData());
                        sendBlockChange(player, block.getRelative(BlockFace.UP).getLocation(), Material.BLACK_CARPET, carpetColor.getWoolData());
    ...
    public void setLoc(int locationId, final Location clicked) {
        ...
        if (type == VisualClaimType.CREATE) {
            if (!bypass && !HCF.getInstance().getServerHandler().isUnclaimed(clicked) && !HCF.getInstance().getServerHandler().isWarzone(player.getLocation())) {
    ...
    public void purchaseClaim() {
        ...
        if (corner1 != null && corner2 != null) {
            ...
            if (x >= 50 && z >= 50 && !bypass){
                player.sendMessage(CC.translate("&cYou cannot go over the 50 block claim limit."));
                return;
            }
            ...
            claim.setName(playerTeam.getName() + "_" + (100 + HCF.RANDOM.nextInt(800)));
            claim.setY1(0);
            claim.setY2(256);
