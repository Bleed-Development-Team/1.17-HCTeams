...
public class MapListener implements Listener {
    ...
    public void onBlockBreak(BlockBreakEvent event) {
        if (!HCF.getInstance().getMapHandler().isFastSmeltEnabled() || event.getPlayer().getItemInHand() == null || !event.getPlayer().getItemInHand().getType().name().contains("PICKAXE") || event.getPlayer().getItemInHand().containsEnchantment(Enchantment.SILK_TOUCH)) {
            return;
        }
    ...
    public void onProjectileLaunch(ProjectileLaunchEvent event) {
        ...
        if (inHand != null && inHand.getType() == Material.EXPERIENCE_BOTTLE && inHand.hasItemMeta() && Objects.requireNonNull(inHand.getItemMeta()).hasLore() && inHand.getItemMeta().getLore().size() == 1) {
    ...
    @EventHandler
    public void potionPRevention(PlayerItemConsumeEvent event) {
        ...
        if (potion.getType() == Material.POTION) {
            ...
            if (meta.getBasePotionData().getType().getEffectType() == PotionEffectType.INCREASE_DAMAGE || meta.hasCustomEffect(PotionEffectType.INCREASE_DAMAGE)) {
                event.setCancelled(true);

                player.sendMessage(CC.translate("&cYou can't do that you sticky loser!"));
            } else if (meta.getBasePotionData().getType().getEffectType() == PotionEffectType.DAMAGE_RESISTANCE || meta.hasCustomEffect(PotionEffectType.INCREASE_DAMAGE)){
                event.setCancelled(true);
