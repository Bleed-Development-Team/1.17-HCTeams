...
public class BasicPreventionListener implements Listener {
    ...
    public void onFoodLevelChange(FoodLevelChangeEvent event) {
        ...
        if (event.getFoodLevel() < event.getEntity().getFoodLevel()) {
            ...
            if (HCF.RANDOM.nextInt(100) > (HCF.getInstance().getPvPTimerMap().hasTimer(event.getEntity().getUniqueId()) ? 10 : 30)) {
                event.setCancelled(true);
            }
        }
    }
    ...
    public void onPlace(BlockPlaceEvent event) {
        if (HCF.getInstance().getServerHandler().isSkybridgePrevention() && 110 < event.getBlock().getLocation().getY() && event.getPlayer().getGameMode() != GameMode.CREATIVE) {
    ...
    public void onFish(PlayerInteractEvent event) {
        if (!HCF.getInstance().getServerHandler().isRodPrevention() || (event.getAction() != Action.RIGHT_CLICK_AIR && event.getAction() != Action.RIGHT_CLICK_BLOCK)) {
            return;
        }
