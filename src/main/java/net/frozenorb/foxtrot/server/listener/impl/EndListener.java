...
//            }
    ...
    public static void saveEndReturn() {
        Bukkit.getScheduler().scheduleAsyncDelayedTask(HCF.getInstance(), () -> HCF.getInstance().runRedisCommand(redis -> {
            redis.set("endReturn", HCF.PLAIN_GSON.toJson(endReturn));
            return null;
        }));
    }
    ...
    public static void loadEndReturn() {
        ...
        HCF.getInstance().runRedisCommand(redis -> {
            if (redis.exists("endReturn")) {
                endReturn = HCF.PLAIN_GSON.fromJson(redis.get("endReturn"), Location.class);
            } else {
