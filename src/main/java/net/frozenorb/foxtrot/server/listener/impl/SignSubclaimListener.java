...
public class SignSubclaimListener implements Listener {
    ...
    public void onSignUpdate(SignChangeEvent event) {
        ...
        for (int i = 1; i <= 3; i++) {
            sign.getLine(i);
            if (sign.getLine(i).equalsIgnoreCase(event.getPlayer().getName())) {
                found = true;
                break;
            }
        }
    ...
    public void onBlockBreakChest(BlockBreakEvent event) {
        if (!(event.getBlock().getState() instanceof Chest) || HCF.getInstance().getServerHandler().isUnclaimedOrRaidable(event.getBlock().getLocation()) || HCF.getInstance().getServerHandler().isAdminOverride(event.getPlayer())) {
            return;
        }
    ...
    public void onBlockBreakSign(BlockBreakEvent event) {
        if (!(event.getBlock().getState() instanceof Sign) || HCF.getInstance().getServerHandler().isUnclaimedOrRaidable(event.getBlock().getLocation()) || HCF.getInstance().getServerHandler().isAdminOverride(event.getPlayer())) {
            return;
        }
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        if (event.getAction() != Action.RIGHT_CLICK_BLOCK || !(event.getClickedBlock().getState() instanceof Chest) || HCF.getInstance().getServerHandler().isUnclaimedOrRaidable(event.getClickedBlock().getLocation()) || HCF.getInstance().getServerHandler().isAdminOverride(event.getPlayer())) {
            return;
        }
    ...
    public static void subclaimSigns0(Set<Sign> signs, Block check) {
        for (BlockFace blockFace : OUTSIDE_FACES) {
            ...
            if (relBlock.getType() == Material.OAK_WALL_SIGN || relBlock.getType() == Material.OAK_SIGN) {
