...
public class KOTHCommand extends BaseCommand {
    ...
    public static void kothSchedule(Player sender) {
        ...
        for (Map.Entry<EventScheduledTime, String> entry : HCF.getInstance().getEventHandler().getEventSchedule().entrySet()) {
            ...
            sent++;
            sender.sendMessage(CC.translate("&3&l[KOTH] ") + BLUE + entry.getValue() + WHITE + " can be captured at " + ChatColor.BLUE + KOTH_DATE_FORMAT.format(entry.getKey().toDate()) + WHITE + ".");
        }

        if (sent == 0) {
            sender.sendMessage(CC.translate("&3&l[KOTH] ") + WHITE + "KOTH Schedule: " + DARK_RED + "Undefined");
        } else {
            sender.sendMessage(CC.translate("&3&l[KOTH] ") + WHITE + "It is currently " + ChatColor.BLUE + KOTH_DATE_FORMAT.format(new Date()) + WHITE + ".");
        }
    }
    ...
    public static void koth(Player sender) {
        ...
        for (Map.Entry<EventScheduledTime, String> entry : HCF.getInstance().getEventHandler().getEventSchedule().entrySet()) {
            if (entry.getKey().toDate().after(now)) {
                sender.sendMessage(CC.translate("&3&l[KOTH] ") + BLUE + entry.getValue() + WHITE + " can be captured at " + BLUE + KOTH_DATE_FORMAT.format(entry.getKey().toDate()) + WHITE + ".");
                sender.sendMessage(CC.translate("&3&l[KOTH] ") + WHITE + "It is currently " + BLUE + KOTH_DATE_FORMAT.format(now) + WHITE + ".");
                sender.sendMessage(YELLOW + "Type '/koth schedule' to see more upcoming Events.");
                return;
            }
        }

        sender.sendMessage(CC.translate("&3&l[KOTH] ")+ WHITE + "Next Event: " + DARK_RED + "Undefined");
    }
