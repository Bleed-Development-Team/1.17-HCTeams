...
public class ResistanceTwoAbility extends Ability {
    @Override
    public String getName() {
        return "&e&lResistance III";
    }
    ...
    public String getDescription() {
        return "Right click to receive 3 seconds of Resistance III.";
    }
    ...
    public void interact(PlayerInteractEvent event){
        ...
        if (event.getAction() == Action.RIGHT_CLICK_BLOCK || event.getAction() == Action.RIGHT_CLICK_AIR) {
            if (!isSimilarTo(player.getItemInHand(), Items.getResistance())) return;
            if (!canUse(player)) return;

            int i = 0;
            for (Player friendly : getNearbyPlayers(player, true)) {
                friendly.addPotionEffect(new PotionEffect(PotionEffectType.DAMAGE_RESISTANCE, 20 * 3, 2));
                i++;
            }
            ...
            player.sendMessage(CC.translate("&c❤ &6You have gave " + (i == 0 ? "yourself" : "&f" + i + " &6player" + (i == 1 ? "" : "s")) + "  &fResistance III&6."));
        }
    }
