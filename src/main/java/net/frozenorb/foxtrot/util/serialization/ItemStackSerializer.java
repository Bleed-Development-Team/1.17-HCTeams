package net.frozenorb.foxtrot.util.serialization;
...
public final class ItemStackSerializer {
    ...
    public static BasicDBObject serialize(ItemStack itemStack) {
        if (itemStack == null || itemStack.getType() == Material.AIR)
            return AIR;
        BasicDBObject item = (new BasicDBObject("type", itemStack.getType().toString())).append("amount", itemStack.getAmount()).append("data", Objects.requireNonNull(itemStack.getData()));
        BasicDBList enchants = new BasicDBList();
        for (Map.Entry<Enchantment, Integer> entry : itemStack.getEnchantments().entrySet())
            enchants.add((new BasicDBObject("enchantment", entry.getKey().getKey())).append("level", entry.getValue()));
    ...
    public static ItemStack deserialize(BasicDBObject dbObject) {
        ...
        if (dbObject.containsField("enchants")) {
            ...
            for (Object o : enchs) {
                ...
                item.addUnsafeEnchantment(Objects.requireNonNull(Enchantment.getByKey(NamespacedKey.fromString("enchantment"))), enchant.getInt("level"));
            }
        }
    ...
    static {
        AIR.put("type", "AIR");
        AIR.put("amount", 1);
        AIR.put("data", Integer.valueOf(0));
    }
}